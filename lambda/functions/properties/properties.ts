...
function generateRandomGeoJSONProperties(): any {
  ...
  const randomDate = () =>
    new Date(Date.now() - randomInt(0, 1e10)).toISOString();
  ...
  return {
    ...
    tags: randomArray(
      ["landmark", "park", "historical", "museum", "mountain"],
      randomInt(1, 3)
    ),
    details: {
    ...
    accessibility: {
      ...
      languagesAvailable: randomArray(
...
export function populateGeoJsonFeatureWithProperties(
  feature: Feature
): Feature {
  feature = {
    ...feature,
