...
export function MultiPoints(): OkResponse {
  ...
  export function RandomMultiPoints(): OkResponse {
    ...
    const coordinates: [number, number][][] = points.features.map((feature) => {
      if (feature.geometry.type === "Point") {
        const [x, y] = feature.geometry.coordinates as [number, number];
        return [[x, y], [x + 0.01, y]]; // Offset the second point slightly to the right
      }
      return [];
    });
  
    // Flatten the array for MultiPoint creation
    const flattenedCoordinates = coordinates.flat();
  
    const multipoint = turf.multiPoint(flattenedCoordinates);
    return GetOkResponse(multipoint);
  }
